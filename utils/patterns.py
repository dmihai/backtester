...
def add_engulfing(data, pip_value=0.0001, min_pips=10, room_left=4, room_divisor=5.0):
    df = data.copy()
    ...
    # check if there is space to the left
    for i in range(2, room_left+2):
        df.loc[(df.engulfing == -1) & (df.shift(i).high_price > df.high_price - room_length) & (df.shift(i).high_price > df.shift(1).high_price - room_length),
               'engulfing'] = 0
        df.loc[(df.engulfing == 1) & (df.shift(i).low_price < df.low_price + room_length) & (df.shift(i).low_price < df.shift(1).low_price + room_length),
               'engulfing'] = 0
